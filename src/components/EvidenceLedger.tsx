...
export function EvidenceLedger({sources, locale}: {sources: ResolvedEvidenceSource[]; locale: Locale}) {
  ...
  return (
    <section className="evidence-ledger" aria-busy={allRunning || checkingCount > 0}>
      <span className="sr-only" role="status" aria-live="polite" aria-atomic="true">{liveMessage}</span>
      <div className="evidence-heading"><div><p className="eyebrow">{labels.eyebrow}</p><h3>{labels.title}</h3><p>{labels.body}</p></div><button type="button" className="outline-button" onClick={verifyAll} disabled={allRunning} aria-busy={allRunning}>{allRunning ? labels.verifyingAll : `${labels.verifyAll} (${sources.length})`}</button></div>
